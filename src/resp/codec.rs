...
use crate::resp::{parse::parse, RespValue};
...
mod tests {
    ...
    fn encode_simple_string() {
        ...
        let item = RespValue::SimpleString("OK".into());
    ...
    fn encode_simple_error() {
        let mut buf = BytesMut::with_capacity(1024);
        let item = RespValue::SimpleError("Error message".into());
        RespFrame::encode_value(item, &mut buf).unwrap();
    ...
    fn encode_bulk_string() {
        ...
        // empty string
        let item = RespValue::BulkString("".into());
        RespFrame::encode_value(item, &mut buf).unwrap();
        assert_eq!(&buf[..], &b"$0\r\n\r\n"[..]);
        buf.clear();

        // basic bulk string
        let item = RespValue::BulkString("hello".into());
        RespFrame::encode_value(item, &mut buf).unwrap();
        assert_eq!(&buf[..], &b"$5\r\nhello\r\n"[..]);
    }
